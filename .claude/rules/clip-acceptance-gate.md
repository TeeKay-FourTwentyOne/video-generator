...
**clone-check calibration (PERSONAL BEST, 2026-08-19): heavy-splash water scenes false-positive.** Three high-confidence (0.90–0.95) `clone_body` claims in one build — all three false at full resolution: upturned feet/limb clusters + foam-bisected bodies in violent splash parse as "a second swimmer," and the rule-out prose fabricates corroborating detail ("both red caps visible" when only one exists). Its true-positive domain remains dry-land / cross-frame moves (the MINE TOO class). The verdict procedure is unchanged and is the point: **the saved strips (and full-res crops of the flagged window) are the arbiter, never the tool's confidence or its rule-out reasoning.** A splash-window flag earns frame-level verification, not automatic reroll — and not automatic dismissal either.
