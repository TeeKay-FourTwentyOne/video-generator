...
- **clone-check** is the post-generation catcher for *subject duplication during motion* (a clone/ghost-twin that merges as the subject moves) — a class neither anchor-drift nor clip-qa reliably sees. anchor-drift is pre-gen frames; clone-check is post-gen motion. Both feed the `.claude/rules/clip-acceptance-gate.md` gate.
