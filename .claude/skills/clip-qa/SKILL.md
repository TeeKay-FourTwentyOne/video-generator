...
- **Misses subject duplication during motion.** A clone / ghost-twin / merge — Veo briefly rendering a SECOND copy of the same subject as they stand or cross frame — lives ~0.5s inside a fast-motion window, and this tool's even cadence + motion-tolerant prompt reads it as normal motion and lets it through (validated: it missed the MINE TOO shot6 clone). **That class is `clone-check`'s job** (motion-gated dense sampling + a high-recall duplication prompt). Run `clone-check` alongside this on any motion-bearing shot.
